...
import type { SpriteAsset, SpriteDragPayload } from './SpriteModal';
...
const SpriteLibrary = ({ setIsSpriteModalOpen }: Props) => {
  ...
  const [activeSpriteCategory, setActiveSpriteCategory] = useState('all');
  const addSpriteToGame = useEditorStore((state) => state.addSpriteToGame);
  const spriteCategories = useMemo(() => ['all', ...new Set(spriteLibrary.map((asset) => asset.category))], []);
  ...
  const filteredSprites = useMemo(() => {
    ...
    return spriteLibrary.filter((sprite) => {
      const matchesCategory = activeSpriteCategory === 'all' || sprite.category === activeSpriteCategory;
      const matchesSearch =
        !query ||
  ...
  }, [searchQuery, activeSpriteCategory]);

  const handleSpriteClick = async (sprite: SpriteAsset) => {
    const success = await addSpriteToGame(buildPayload(sprite), { x: 0, y: 0 });
          ...
          <div className="flex flex-wrap gap-4">
            {filteredSprites.map((sprite) => (
              <div
                ...
                onClick={() => {
                  console.log('[SpriteLibrary] handleSpriteClick() called', sprite);
