...
export function NumericField({ label, icon, value, disabled, min, max, onChange, onBlur }: NumericFieldProps) {
...
const SpritePosition = ({ borderless }: { borderless?: boolean }) => {
  ...
  return (
    <div className='w-full flex flex-col gap-2 text-xs'>
      <div className='flex items-center gap-5'>
        ...
        <NumericField
          ...
          onBlur={() => {
            ...
            if (selectedSprite!.x !== finalValue) updateSpriteInstance(selectedSpriteId!, { x: finalValue as number });
          }}
        />
        <NumericField
          label='y'
      ...
      <div className='flex items-center gap-5'>
        <fieldset className='flex items-center gap-y-0 gap-x-1 border-0 p-0 m-0'>
          <legend className={`${labelClasses} mr-2 sr-only`}>Show sprite</legend>
          ...
          <button
            ...
            className={`${visibilityButtonBaseClasses} rounded-l-md ${
              selectedSprite?.visible !== false ? visibilityButtonActiveClasses : visibilityButtonInactiveClasses
            }`}
            title='Show sprite'
          >
