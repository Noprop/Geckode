...
export const authApi = {
  ...
  getUserDetails: () => api.get<Required<User>>(`${USERS_API_URL}user-details/`).then(res => res.data as User).catch(() => null),
  getAccessToken: () => api.get<string>(`token/`).then(res => res.data.access_token),
}