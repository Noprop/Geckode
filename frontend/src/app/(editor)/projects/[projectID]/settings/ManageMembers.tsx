...
export const ManageMembers = ({ prj, setPrj, user }: Props) => {
  ...
  const ListUserInvite = ({ user }: { user: User }): ReactElement => {
    ...
    useEffect(() => {
      ...
      tableRef.current?.data.forEach((prjCollaborator) => {
        if (prjCollaborator.collaborator.id === user.id) {
          setStatus("already-member");
          return;
        }
      });
      if (prj.owner.id === user.id) setStatus("already-member");
    ...
    if (status === "standard") {
      prjInvites.forEach((invitedUser) => {
