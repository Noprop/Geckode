...
export const ManageCollaborators = ({ prj, setPrj, user, userPerm }: Props) => {
  ...
  const getCurrentInvites = () => {
    projectsApi(prj.id)
      ...
      .catch((err) => snackbar(extractAxiosErrMsg(err, 'Failed to get project invites'), 'error'));
  };
        ...
        actions={[
          {
            ...
            canUse: (a) => ['admin', 'invite', 'owner'].includes(userPerm), // WILL NEED TO CHANGE TO PROPER PERMISSIONS!!!
          },
        ]}
        extras={
          <>
            <Button
              ...
              disabled={['admin', 'invite', 'owner'].includes(userPerm)}
            >
              Invite Users
            </Button>
          </>
