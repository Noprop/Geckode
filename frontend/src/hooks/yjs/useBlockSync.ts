...
export const useBlockSync = (documentName: string) => {
  ...
  useEffect(() => {
    if (currentSpriteId) {
      console.log('changing the blocks map', 'blocks-' + currentSpriteId);
      //setBlocksMap(doc.getMap('blocks-' + currentSpriteId));
      setBlocksMap(doc.getMap('blocks'));
    }
  }, [currentSpriteId]);

  const blockEventsListener = useCallback(
    (event: Blockly.Events.Abstract) => {
      ...
      const blocks = getBlocksFromSerializedBlock(
        serializeBlock(blockEvent.blockId ?? '', blocklyWorkspace),
        blocklyWorkspace,
  ...
  const blocksMapObserver = useCallback(
    (event: YMapEvent<Block>, transaction: Transaction) => {
